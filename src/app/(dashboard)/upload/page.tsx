...
//         // Cari bagian ini di fungsi onDrop kamu:
//         if (!previewRes.ok) {
//           const errorData = await previewRes.json().catch(() => null)
//           const errorMsg = errorData?.detail || await previewRes.text()
          
//           // PERBAIKAN: Jangan gunakan 'throw new Error', tapi set result secara langsung
...
//           setFile(null) // Reset file agar user bisa upload ulang
//           setIsParsing(false)
...
//         // PERBAIKAN 2: Tangkap error format (list index out of range) dan jadikan ramah pengguna
...
//         // Kosongkan file jika gagal preview
//         setFile(null)
//       } finally {
//         setIsParsing(false)
//       }
...
//       // PERBAIKAN 1: Reset semua state preview setelah berhasil upload
//       setFile(null)
//       setCleanedData([])
//       setAllData([])
...
//       // Perbarui statistik atas secara otomatis
//       fetchDbStats()
...
//       {/* Result Message (Dipindah ke atas Upload Box agar mudah terlihat saat error) */}
//       {result && (
...
//                       <tr key={log.id} className="border-b last:border-b-0 hover:bg-slate-50 transition-colors">
...
//                   {showFilters && (
//                     <div className="grid grid-cols-2 md:grid-cols-5 gap-3 p-4 bg-slate-50 border border-slate-100 rounded-xl">
//                       <div>
//                         <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1 block">Retailer</label>
//                         <Select 
...
//                   className="w-full h-12 bg-[#054CC7] hover:bg-blue-800 text-white font-bold text-base rounded-xl shadow-lg shadow-[#054CC7]/20 transition-all active:scale-[0.98]"
//                 >
//                   {isLoading ? (
//                     <>
//                       <Loader2 className="h-5 w-5 mr-2 animate-spin" />
//                       Menyimpan ke Database... {uploadProgress}%
//                     </>
//                   ) : (
//                     <>
//                       <Database className="h-5 w-5 mr-2" />
//                       Simpan {cleanedData.length} Data ke Database
//                     </>
//                   )}
//                 </Button>
//               </div> */}

//               {/* Ganti bagian Button Simpan di bagian bawah dengan ini */}
//               <div className="pt-4">
//                 {!showConfirmUpload ? (
//                   <Button 
//                     onClick={() => setShowConfirmUpload(true)} // Tampilkan konfirmasi dulu
//                     disabled={isLoading || isParsing || cleanedData.length === 0}
...
export default function UploadPage() {
  ...
  // Fungsi Fetch Statistik Database (Ditambahkan filter parameter untuk STAFF)
  const fetchDbStats = async () => {
    setIsLoadingDb(true)
    try {
  ...
  // Panggil data stats hanya saat session sudah ready
  useEffect(() => {
  ...
  //     const formData = new FormData()
  //     formData.append('file', file)
  //     formData.append('uploaded_by', session?.user?.name || 'Unknown') // Tambahkan nama pengunggah
  ...
  const handleUpload = async () => {
    ...
    try {
      const formData = new FormData()
      formData.append('file', file)
      // Note: Backend python mungkin tidak pakai uploaded_by, tapi kita tetap kirim jika dibutuhkan
      ...
      // ==========================================
      // KODE BARU: Catat ke tabel upload_history
      // ==========================================
      if (data.status === 'success') {
        await fetch('/api/upload/history', {
          ...
          body: JSON.stringify({
            file_name: file.name,
            system_name: 'Artavista', // Nama sistem bisa disesuaikan
            status: 'success',
        ...
        }).catch(err => console.error("Gagal menyimpan log history:", err)); // Abaikan jika gagal simpan log agar tidak mengganggu UI
      }
      // ==========================================
      ...
      fetchDbStats() // Update statitik angka di atas
      ...
      // Jika terjadi error, catat juga sebagai "failed" di history
      if (file) {
            ...
            {isLoadingLogs ? (
              ...
              <div className="overflow-x-auto border rounded-lg">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {uploadLogs.slice(0, 10).map((log) => (
                      <tr key={log.id_upload} className="border-b last:border-b-0 hover:bg-slate-50 transition-colors">
                        <td className="p-3 font-medium">{log.file_name}</td>
                        <td className="p-3 text-gray-500">{log.system_name}</td>
                        <td className="p-3 text-right font-semibold">{log.total_rows?.toLocaleString()}</td>
                        <td className="p-3">
